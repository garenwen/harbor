...
func newAdapter(registry *model.Registry) (a *adapter, err error) {
	...
	log.Debugf("[baidu-ccr.newAdapter] realm=%s, service=%s error=%v", realm, service, err)
	if err != nil {
	...
	clientCache := newClientCache(clusterID, registry.Credential.AccessKey, region)

	var credential = NewAuth(registry.Credential.AccessKey, registry.Credential.AccessSecret, clusterID, clientCache)
...
func ping(registry *model.Registry) (string, string, error) {
	...
	log.Infof("[baidu-ccr.ping] error=%v", err)
	if err != nil {
		return "", "", err
	}
...
func (a *adapter) PrepareForPush(resources []*model.Resource) (err error) {
	log.Debugf("[baidu-ccr.PrepareForPush]")
	for _, resource := range resources {
		...
		log.Debugf("[baidu-ccr.PrepareForPush.createPrivateNamespace] namespace=%s", namespace)
...
func (a *adapter) createPrivateNamespace(namespace string) error {
	...
	if !isExist {
		return cli.CreateNamespace(*a.clusterID, &CreateProjectRequest{
...
func (a *adapter) isNamespaceExists(namespace string) (bool, error) {
	...
	_, err = cli.GetNamespace(*a.clusterID, namespace)
...
func (a *adapter) listAllNamespaces() ([]string, error) {
	...
	for {
		resp, err := cli.ListNamespaces(*a.clusterID, pageNo, pageSize)
		if err != nil {
...
func (a *adapter) listReposByNamespace(namespace string) ([]*RepositoryResult, error) {
	...
	for {
		resp, err := cli.ListReposByNamespace(*a.clusterID, namespace, pageNo, pageSize)
		if err != nil {
...
func (a *adapter) getImages(namespace, repo, tag string) ([]*TagResult, []string, error) {
	...
	for {
		resp, err := cli.GetImage(*a.clusterID, namespace, repo, tag, pageNo, pageSize)
		if err != nil {
...
func (a *adapter) deleteImage(namespace, repo, tag string) error {
	...
	return cli.DeleteImage(*a.clusterID, namespace, repo, tag)
}
