...
type ccrChartVersionDetail struct {
	Metadata *ccrChartVersionMetadata `json:"metadata"`
}
type ccrChartVersionMetadata struct {
	URLs []string `json:"urls"`
}
...
func (a *adapter) FetchCharts(filters []*model.Filter) (resources []*model.Resource, err error) {
	log.Debugf("[baidu-ccr.FetchCharts]filters: %#v", filters)
	// 1. list namespaces
...
func (a *adapter) ChartExist(name, version string) (exist bool, err error) {
	log.Infof("[baidu-ccr.ChartExist] name=%s version=%s", name, version)
...
func (a *adapter) DownloadChart(name, version, contentURL string) (rc io.ReadCloser, err error) {
	log.Infof("[baidu-ccr.DownloadChart] start download chart %s", name)
	var info *ccrChartVersionDetail
	...
	log.Infof("[baidu-ccr.DownloadChart] download chart url: %s", url)
	...
	if resp.StatusCode != http.StatusOK {
		...
		err = fmt.Errorf("[baidu-ccr.DownloadChart.failed] chart=%s, status=%d, body=%s", req.URL.String(), resp.StatusCode, string(body))
...
func (a *adapter) UploadChart(name, version string, reader io.Reader) (err error) {
	log.Infof("[baidu-ccr.UploadChart] start upload chart %s", name)
	var namespace string
	var chart string
	namespace, chart, err = parseChartName(name)
	if err != nil {
	...
	log.Infof("[baidu-ccr.UploadChart] upload chart url: %s", url)
	var req *http.Request
	var resp *http.Response
...
func parseChartName(name string) (namespace, chart string, err error) {
	...
	return "", "", fmt.Errorf("[baidu-ccr.parseChartName.invalid_name] name=%s", name)
}
